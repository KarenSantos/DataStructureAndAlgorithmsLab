...
public class BTreeImpl<T extends Comparable<T>> implements
	...
	private void split(BNode<T> node){
		...
		node.getParent().addElement(elemMediano);       // subindo mediana para o pai
		int index = node.getParent().indexOfChild(node);
		
		node.getParent().getChildren().set(index, left);  // filhos do pai agora vao incluir left e right
		node.getParent().getChildren().add(index+1, right);
		
		left.setParent(node.getParent());  // setando pai de left e right
